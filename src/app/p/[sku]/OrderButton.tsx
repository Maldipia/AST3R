...
export default function OrderButton({ sku, name, price, salePrice, imageUrl, inStock, sizes = [] }: Props) {
  ...
  const doAdd = () => {
    ...
    toast.success(`✓ Added — Size ${selected || ''}`, { duration: 2200 });
  ...
  return (
    <div id="order-buttons" className="space-y-2.5">

      {/* Inline size picker — shown INSIDE the action area if sizes exist */}
      {hasSizes && (
