...
export default async function ProductPage({ params }: { params: { sku: string } }) {
  ...
  // Fetch product early — bail if not found
  ...
  const [priceRes, sizeRes, relatedRes, reviewsRes] = await Promise.all([
    ...
    supabase.from('products')
      ...
      .eq('status', 'active').eq('category', rawProduct.category).neq('sku', sku).limit(4),
    supabase.from('reviews')
      .select('customer_name, rating, comment, created_at')
      .eq('sku', sku).eq('verified', true).order('created_at', { ascending: false }),
  ]);

  const comparePrice   = priceRes.data?.compare_price as number | null ?? null;
  ...
  const totalSizeQty = hasSizes
    ? sizeInventory.reduce((sum: number, s: any) => sum + s.quantity, 0)
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 min-h-[calc(100vh-4rem)]">
            ...
            <div className="relative bg-brand-cream overflow-hidden" style={{ minHeight: '60vw', maxHeight: '90vh' }}>
              {rawProduct.image_url ? (
                <Image src={rawProduct.image_url} alt={rawProduct.name}
            ...
            <div className="flex flex-col justify-center px-8 py-16 lg:px-16 xl:px-24 page-enter">
              <div className="flex items-center gap-2 mb-8">
                ...
                <span className="text-xs tracking-widest uppercase text-brand-gray font-medium">{rawProduct.category}</span>
              </div>

              <h1 className="display-lg text-brand-black mb-6 leading-tight">{rawProduct.name}</h1>

              {/* Price */}
              <div className="mb-6">
                <div className="flex items-baseline gap-3 flex-wrap">
                  {comparePrice && comparePrice < rawProduct.price ? (
                    <>
                      <span className="font-serif text-3xl font-medium text-red-600">
                        {formatPrice(comparePrice, rawProduct.currency)}
                      </span>
                      <span className="font-serif text-xl text-brand-gray line-through">
                        {formatPrice(rawProduct.price, rawProduct.currency)}
                      </span>
                      <span className="text-xs font-bold text-white bg-red-500 px-2 py-1">
                        {Math.round((1 - comparePrice / rawProduct.price) * 100)}% OFF
                      </span>
                    </>
                  ) : (
                ...
                {comparePrice && comparePrice < rawProduct.price && (
                  <p className="text-xs text-green-600 mt-1 font-medium">
                    You save {formatPrice(rawProduct.price - comparePrice)}
                  </p>
                )}
              </div>
              ...
              {/* Sizes */}
              {hasSizes && sizeInventory.length > 0 && (
                <div className="mb-8">
                  <p className="text-xs font-medium tracking-widest uppercase text-brand-gray mb-3">Select Size</p>
                  <div className="flex flex-wrap gap-2">
                    {sizeInventory.map((si: any) => (
                      <div key={si.size} className={`flex flex-col items-center px-4 py-2 border text-sm font-medium min-w-[56px] ${
                        si.quantity <= 0
                        ...
                        <span className={`text-xs mt-0.5 font-normal ${
                          si.quantity <= 0 ? 'text-brand-light' : si.quantity <= 3 ? 'text-orange-500' : 'text-brand-gray'
                        }`}>
              ...
              <p className="text-brand-gray text-sm leading-relaxed mb-10 max-w-md">{rawProduct.description}</p>
              ...
              <QRDownload sku={rawProduct.sku} productName={rawProduct.name} />
            </div>
          </div>
        </div>
